...
def	log_train_psnr(result, imsource, loss, writer, epoch, idx, num_minibatches, training_params, sigma=None):
	'''Logs trai loss.
	'''
...
# 		  training_params['step'])
	print("[epoch {}][{}/{}] loss: {:1.4f} sigma: {:1.4f} PSNR_train: {:1.4f}".\
		  format(epoch+1, idx+1, num_minibatches, loss.item(), sigma, psnr_train))
...
def validate_and_log(model_temp, dataset_val, valnoisestd, temp_psz, writer, \
	...
	with torch.no_grad():
		for seq_val in dataset_val:
			...
			out_val = denoise_seq_ivdnet(seq=seqn_val, \
